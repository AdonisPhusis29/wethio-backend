...
if [[ ! -z $NETWORK_ID ]]; then
  case $NETWORK_ID in
    ...
    89 )
      ...
      params="$params --zyn-testnet --gcmode archive --rpcapi db,eth,net,web3,debug"
      ;;
    90 )
...
# data dir
if [[ ! -d $DATA_DIR/zyn ]]; then
  echo "No blockchain data, creating genesis block."
  zyn init $genesisPath --datadir $DATA_DIR 2> /dev/null
fi
...
if [[ $accountsCount -le 0 ]]; then
  ...
  if [[ ! -z $PRIVATE_KEY ]]; then
    ...
    zyn  account import ./private_key \
      --datadir $DATA_DIR \
