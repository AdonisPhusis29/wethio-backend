...
func airDropTokenToAccountNoZyn() {
...
func testTransferTRC21TokenWithAccountNoZyn() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have zyn
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoZyn()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoZyn()
		fmt.Println("Finish transfer trc21 token with a account no zyn")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no zyn")
	}
