...
func loadConfig(file string, cfg *zynConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, zynConfig) {
	// Load defaults.
	cfg := zynConfig{
	...
	if ctx.GlobalBool(utils.ZynTestnetFlag.Name) {
...
func makeFullNode(ctx *cli.Context) (*node.Node, zynConfig) {
